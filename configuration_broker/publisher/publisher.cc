...
void setUpTopicMap () {
	...
	if (!init) {
		...
		TopicMap[0].cap = WRITE_CONFIG_CAPABILITY(CONFIG1);
		TopicMap[0].handler = update_logger_config;

		TopicMap[1].topic = "rgbled";
		TopicMap[1].cap = WRITE_CONFIG_CAPABILITY(CONFIG2);
...
void update_logger_config(SObj sealedWriteCap, const char *data)
{
	...
	if (set_config(sealedWriteCap, sizeof(LoggerConfig), publish, static_cast<void *>(roConfig)) < 0)
	{
		Debug::log("thread {} Failed to set value for {}", thread_id_get(), sealedWriteCap);
	};
...
void __cheri_compartment("publisher") updateConfig(const char* topic, const char* message)
{
	...
	if (!found) {
		Debug::log("thread {} Unexpected Message topic {}", thread_id_get(), topic);
	}
};
...
void __cheri_compartment("publisher") bad_dog()
{
	while (true)
	{
		...
		sleep(8000);
		Debug::log("-----------------------------");
		Debug::log("thread {} Sending oversize data for {}", thread_id_get(), CONFIG1);
		void *d1 = malloc(100);
		set_config(WRITE_CONFIG_CAPABILITY(CONFIG1), sizeof(d1), publish, d1);
		free(d1);

		// Undersized value which might break the consumer 
		sleep(8000);
		Debug::log("-----------------------------");
		Debug::log("thread {} Sending undersize data for {}", thread_id_get(), CONFIG1);
		void *d2 = malloc(4);
		set_config(WRITE_CONFIG_CAPABILITY(CONFIG1), sizeof(d2), publish, d2);
		free(d2);
	}
};
