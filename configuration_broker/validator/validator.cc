...
DEFINE_VALIDATE_CONFIG_CAPABILITY(LOGGER_CONFIG);
#define RGB_LED_CONFIG "rgb_led"
DEFINE_VALIDATE_CONFIG_CAPABILITY(RGB_LED_CONFIG);
...
bool __cheri_callback validateLogger(void *data)
{
	...
	if (!valid)
	{
		Debug::log("thread {} Logger Config validation failed",
		           thread_id_get());
	}
	return valid;
}
...
void __cheri_compartment("validator") ValidatorInit()
{
	...
	set_validator(VALIDATE_CONFIG_CAPABILITY(LOGGER_CONFIG), validateLogger);
}